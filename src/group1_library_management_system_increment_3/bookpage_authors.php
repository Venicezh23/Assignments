...
<body>
    ...
    <div id="myModalHelp" class="modal">
        ...
        <div class="group-available-books">
            <!-- Authors -->
            <div class="availablebooks" id="authors">
                <?php
                    $select_author = $conn->prepare("SELECT DISTINCT `Name` FROM `author` ORDER BY `Name` $order");
                ...
                <?php
                    $select_books_amount = $conn->prepare("SELECT COUNT(*) AS `ROW_COUNT` FROM (SELECT `book`.*, `author`.Name FROM `book` JOIN `author` ON `book`.`BookID` = `author`.`BookID` WHERE `author`.Name = :authorName AND `book`.`status`<>'Archived' GROUP BY `book`.`ISBN`) AS subquery_alias;");
                    $select_books_amount->bindParam(':authorName', $fetch_author['Name'], PDO::PARAM_STR);
