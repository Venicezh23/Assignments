...
<body>
    <div class="main-body">
        ...
        <div class="navbar">
            <a class="back-text" href="homepage.php">
        ...
        <div class="big-container">
            <div class="profile-container">
                ...
                ?>
                <img src="https://pbs.twimg.com/media/GH4y2jfWkAAteDX?format=png&name=360x360" class="profile-pic" />
                <div class="profile-name"><?= $fetch_user['LastName']; ?> <?= $fetch_user['FirstName']; ?></div>
                <div class="profile-usertype"><?= $fetch_user['Type']; ?> (<?= $fetch_user['Course']; ?>)</div>
                ...
                <div class="row-container" style="margin-top: 2rem;">
                    <div class="title-text">
                        <i class="fa-solid fa-book fa-2xl"></i> Book(s) Borrowed
                    </div>
                <?php
                    $select_borrow = $conn->prepare("SELECT COUNT(*) AS Count FROM `book_issued` WHERE `UserID` = '".$_SESSION['ID']."';");
                    $select_borrow->execute();
                    if($select_borrow->rowCount() > 0){
                        while($fetch_borrow = $select_borrow->fetch(PDO::FETCH_ASSOC)){
                ?>
                        <div class="num-text"><?= $fetch_borrow['Count']; ?></div>
                <?php
                        }
                    }else{
                        ?>
                ...
                <div class="row-container">
                    <div class="title-text">
                        <i class="fa-regular fa-calendar-check fa-2xl"></i> Book(s) Reserved
                    </div>
                <?php
                    $select_reserve = $conn->prepare("SELECT COUNT(*) AS Count FROM `book_reserve` WHERE `UserID` = '".$_SESSION['ID']."';");
                    $select_reserve->execute();
                    if($select_reserve->rowCount() > 0){
                        while($fetch_reserve = $select_reserve->fetch(PDO::FETCH_ASSOC)){
                ?>
                        <div class="num-text"><?= $fetch_reserve['Count']; ?></div>
                <?php
                        }
                    }else{
                        ?>
                ...
                <div class="row-container">
                    <div class="title-text">
                        <i class="fa-solid fa-receipt fa-2xl"></i> Fine(s) Pay/Paid (RM)
                    </div>
                <?php
                    $select_fine = $conn->prepare("SELECT SUM(FineAmount) AS TotalFine FROM `fine` WHERE `UserID` = '".$_SESSION['ID']."';");
                    $select_fine->execute();
                    if($select_fine->rowCount() > 0){
                        while($fetch_fine = $select_fine->fetch(PDO::FETCH_ASSOC)){
                ?>
                        <div class="num-text"><?= number_format($fetch_fine['TotalFine'], 2); ?></div>
                <?php
                        }
                    }else{
                        ?>
