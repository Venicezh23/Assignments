...
<head>
    <meta charset="UTF-8">
    <title>Book Page (Publishers)</title>
...
<body>
    ...
    <div id="myModalHelp" class="modal">
        ...
        <div class="group-available-books">
            <!-- Publisher -->
            <div class="availablebooks" id="publishers">
                <?php
                    $select_publisher = $conn->prepare("SELECT DISTINCT `PublisherName` FROM `book` WHERE `book`.`status`<>'Archived' ORDER BY `PublisherName` $order;");
                    $select_publisher->execute();
                    if($select_publisher->rowCount() > 0) {
                        while($fetch_publisher = $select_publisher->fetch(PDO::FETCH_ASSOC)){
                ?>

                <?php
                    $select_books_amount = $conn->prepare("SELECT COUNT(*) AS `ROW_COUNT` FROM (SELECT `book`.*, `author`.Name FROM `book` JOIN `author` ON `book`.`BookID` = `author`.`BookID` WHERE `PublisherName` = :publisherName AND `book`.`status`<>'Archived' GROUP BY `book`.`ISBN`) AS subquery_alias;");
                    $select_books_amount->bindParam(':publisherName', $fetch_publisher['PublisherName'], PDO::PARAM_STR);
                    $select_books_amount->execute();
                    if($select_books_amount->rowCount() > 0){
                        while($fetch_book_amount = $select_books_amount->fetch(PDO::FETCH_ASSOC)){
                ?>

                <span class="available-books-text">
                    <?= $fetch_publisher['PublisherName']; ?> 
                    <span class="available-books-text" id="numberOfBooks">(<?= $fetch_book_amount['ROW_COUNT']; ?>)</span>
                </span>
                <?php
                }
                }
