...
<body>
    ...
    <div id="myModalHelp" class="modal">
        ...
        <div class="group-available-books">
            <!-- All Books -->
            <div class="availablebooks" id="allBooks">
                <?php
                    $select_books_amount = $conn->prepare("SELECT COUNT(*) AS `ROW_COUNT` FROM ( SELECT DISTINCT `book`.`Title`, GROUP_CONCAT(`author`.Name) AS `Authors` FROM `book` JOIN `author` ON `book`.`BookID` = `author`.`BookID` WHERE `book`.`status`<>'Archived' GROUP BY `book`.`ISBN` ) AS subquery_alias; ");
