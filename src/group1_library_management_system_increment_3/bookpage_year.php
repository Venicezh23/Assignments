...
<body>
    ...
    <div id="myModalHelp" class="modal">
        ...
        <div class="group-available-books">
            <!-- Year -->
            <div class="availablebooks" id="year">
                <?php
                    $select_year = $conn->prepare("SELECT DISTINCT `PublishedYear` FROM `book` WHERE `book`.`status`<>'Archived' ORDER BY `PublishedYear` $order;");
                    $select_year->execute();
                    if($select_year->rowCount() > 0) {
                        while($fetch_year = $select_year->fetch(PDO::FETCH_ASSOC)){
                ?>

                <?php
                    $select_books_amount = $conn->prepare("SELECT COUNT(*) AS `ROW_COUNT` FROM (SELECT `book`.*, `author`.Name FROM `book` JOIN `author` ON `book`.`BookID` = `author`.`BookID` WHERE `PublishedYear` = :publishedYear AND `book`.`status`<>'Archived' GROUP BY `book`.`ISBN`) AS subquery_alias;");
                    $select_books_amount->bindParam(':publishedYear', $fetch_year['PublishedYear'], PDO::PARAM_STR);
