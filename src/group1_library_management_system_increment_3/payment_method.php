...
$sql = "SELECT b.Title, bi.DueDate, bi.DateReturn, f.FineAmount, u.TotalFine, f.BookIssuedID, f.FineType
        FROM book_issued bi 
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if ($buttonClicked == null || $buttonClicked == ''){
		...
		$stmt->bind_param("si", $paymentTypeValue, $_SESSION['ID']);
...
<script>
	...
	function changeCashButton() {
		...
		// Change image source back to original
        if (imageElement3) { //black qr image
			// Set the background of the element using the dynamic URL
			var qrcode = "<?php echo $qrcode; ?>";
			imageElement3.style.background = "url(" + qrcode + ") 50% / contain no-repeat";
        }
	}
    ...
    function goBackOne(){
        window.location.href = 'pay-homepage.php';
