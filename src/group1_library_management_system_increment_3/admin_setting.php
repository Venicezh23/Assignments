<?php
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        $qrCode = $_POST['image-input-hidden'];

        // Check if all input values are float
        if ($qrCode!="") {
            $sql = "UPDATE `setting` SET `QrCodePay` = :qrCode WHERE `QrCodePay` = '".$_SESSION['QrCodePay']."';";
            ...
            if ($success) {
                ...
                $_SESSION['QrCodePay'] = $qrCode;

                $logMessage = "[" . date('Y-m-d H:i:s') . "] User ID: " . $_SESSION['ID'] . " have changed the QR Code payment image.";
...
<body style="background-color:white">
        ...
        <div class="sidebar">
            <nav class="navbar">
                ...
                <div class="navbar-nav w-100">    
                    ...
                    <a href="admin_setting.php" class="nav-item nav-link active"><i class="fa-solid fa-gear"></i>Setting</a>
                </div>
            </nav>
        </div>
        <!-- Sidebar End -->
            ...
            <div class="container-fluid pt-4 px-4">
                <div class="bg-light rounded p-4">
                    ...
                    <div class="align-items-left justify-content-left">
                        ...
                        <form id="edit-form" action="admin_setting.php" method="post">
                            <div class="row g-2 mb-3">
                                <label class="col label-text">Library QR Code Payment</label>
                                <button type="button" class="picture-box" onclick="document.getElementById('qrCode-input').click(); previewImage()">
                                    <img id="preview" style="height: 80px;" src="<?= $_SESSION['QrCodePay'] ?>"/>
                                </button>
                                <input type="file" id="qrCode-input" style="display: none;" accept="image/*" onchange="previewImage();validateImage()"  name="qrCode-input"></input>
                                <input type="hidden" id="image-input-hidden" name="image-input-hidden" value="<?= $_SESSION['QrCodePay'] ?>">
                            </div>    
                            ...
                            <input id="comfirmBtn" type="submit" value="SAVE SETTINGS" class="comfirmBtn">
                        </form>
                        </div>
                    </div>
                </div>
    ...
    <script>
        function previewImage() {
            var fileInput = document.getElementById('qrCode-input');
        ...
        function validateImage() {
            ...
            var file = fileInput.files[0];
            var imgElement = document.getElementById('preview');
            var messageDiv = document.getElementById('message_error');
            var messageSus = document.getElementById('message');
