...
<head>
    <meta charset="UTF-8">
    <title>Book Page (Category)</title>
...
<body>
    ...
    <div id="myModalHelp" class="modal">
        ...
        <div class="group-available-books">
            <!-- Category -->
            <div class="availablebooks" id="category">
                <?php
                    $select_category = $conn->prepare("SELECT DISTINCT `Category` FROM `book` WHERE `book`.`status`<>'Archived' ORDER BY `Category` $order;");
                ...
                <?php
                    $select_books_amount = $conn->prepare("SELECT COUNT(*) AS `ROW_COUNT` FROM (SELECT `book`.*, `author`.Name FROM `book` JOIN `author` ON `book`.`BookID` = `author`.`BookID` WHERE `Category` = :category AND `book`.`status`<>'Archived' GROUP BY `book`.`ISBN`) AS subquery_alias;");
                    $select_books_amount->bindParam(':category', $fetch_category['Category'], PDO::PARAM_STR);
                ...
                <div class="book-box">
                    <?php
                        $categorytype = $fetch_category['Category'];
                        ...
                        $select_books->bindParam(':category', $categorytype, PDO::PARAM_STR); 
